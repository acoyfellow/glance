...
const lanes: Record<string, any> = {
  ".context": { name: ".context", state: "active", risk: "low", why: "local observation substrate", finding: "watch/orb/menu bar observe local repo, file, conversation, and harness signals", next: "keep trimming runner-era controls and make signals quieter", source: ".context" },
  ...
  "deja": { name: "deja", state: "active", risk: "low", why: "local-first agent memory plus tiny async mailbox", finding: "MCP memory/handoff remains; added mailbox MVP: send/inbox/read/reply over SQLite, CLI supports `deja as <author> ...`; tests/typecheck green", next: "dogfood Pi ↔ OpenCode via MCP inbox; then consider live receiver adapters without bloating repo", source: "2026-05-01 pi session" },
  ...
  "t2t": { name: "t2t", state: "watch", risk: "low", why: "voice cockpit through MCP", finding: "moved into portfolio root; not configured for Glance", next: "later add an observation surface if useful", source: "portfolio" },
